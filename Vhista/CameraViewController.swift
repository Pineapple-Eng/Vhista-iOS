...
//        VhistaSpeechManager.shared.sayGreetingMessage()r
    ...
    @IBAction func makeDeepAnalysis(_ sender: Any) {
        ...
        if stillImageOutput.connection(with: AVMediaType.video) != nil{
            ...
            let deviceDescoverySession = AVCaptureDevice.DiscoverySession(deviceTypes: [.builtInWideAngleCamera], mediaType: AVMediaType.video, position: .back)
...
extension CameraViewController {
    ...
    func setUpCamera() {
        ...
        guard let camera = AVCaptureDevice.default(for: .video) else {
            ...
            VhistaSpeechManager.shared.sayText(stringToSpeak: NSLocalizedString("No_Camera", comment: "Let the user know there is no camera in the device"), isProtected: true, rate: Float(0.6))
            UINotificationFeedbackGenerator().notificationOccurred(.error)
            return
        }
