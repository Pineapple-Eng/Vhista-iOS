...
public class RippleView: UIView {
    ...
    /// if the forcedMaximumCircleRadius is used the number of drawn circles could be less than numberOfCircles
    ...
    /// The minimum radius of circles, used to make space between the disk and the first circle, the radius must be grather than 5px,
    /// because if not the first circle will not be shown, the default value is 10, it's recommanded to use a value grather than the disk radius
    /// if you would like to show circles outside disk
...
extension RippleView {
    ...
    public func startAnimation() {
        ...
        circlesAnimationTimer =  Timer.scheduledTimer(timeInterval: timeInterval,
                                                      target: self,
                                                      selector: #selector(animateCircles),
                                                      userInfo: nil,
                                                      repeats: true)
        diskAnimationTimer = Timer.scheduledTimer(timeInterval: timeInterval,
                                                  target: self,
