...
final public class RadarView: RippleView {
    ...
    public override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        guard let index = itemsLayer.firstIndex(where: {
            let itemLayer = $0.layer
            let frame = CGRect(x: itemLayer.position.x - itemLayer.bounds.midX,
                               y: itemLayer.position.y - itemLayer.bounds.midY,
                               width: itemLayer.bounds.width,
                               height: itemLayer.bounds.height)
...
extension RadarView {
   ...
   public func add(item: Item, using animation: CAAnimation = Animation.transform()) {
    ...
    public func remove(item: Item) {
        guard let index = itemsLayer.firstIndex(where: { $0.item.uniqueKey == item.uniqueKey }) else {
